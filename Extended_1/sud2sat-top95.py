...
def Encode_variable(puzzle,dimension_size):
        row = 1
        col = 1
        count = 0
...
def cons_OneNumPerEntry(dimension_size,variable_list,puzzle):
    output = ''
...
def cons_OneNumPerRow(dimension_size):
    output = ''
...
def cons_OneNumPerCol(dimension_size):
    output = ''
...
def cons_OneNumPerBlock(dimension_size):
    output = ''
...
def write_CNF(puzzle,dimension_size,targetfile):
    variable_list = Encode_variable(puzzle,dimension_size)
    outfile = open(targetfile, "w")
    outfile.write(pnf_GeneralInfo(dimension_size))
    outfile.write(cons_OneNumPerEntry(dimension_size,variable_list,puzzle))
    outfile.write(cons_OneNumPerRow(dimension_size))
    outfile.write(cons_OneNumPerCol(dimension_size))
    outfile.write(cons_OneNumPerBlock(dimension_size))
...
def main():
    ...
    if (len(sys.argv) != 3):  # the input content must have more than two words
        print("Usage: ./sud2sat-top95.py <input filename> --top95")  # make sure type '-' in front of the file name.
        return
